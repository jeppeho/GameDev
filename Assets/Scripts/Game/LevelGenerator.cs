...
//	}
	...
	private void CreateWoodCubes(int z){

		Vector3 position1 = new Vector3 (woodSteppingStones1 [z], 0.2f, z);
		Vector3 position2 = new Vector3 (woodSteppingStones2 [z], 0.2f, z);

		GameObject cube1 = Instantiate (woodCube, position1, Quaternion.identity) as GameObject;
		GameObject cube2 = Instantiate (woodCube, position2, Quaternion.identity) as GameObject;
	}

	private void CreateWaterSteppingStone(float[] noise, int z){
		GameObject cube = Instantiate (woodCube, new Vector3(noise[z], 0.2f, z), Quaternion.identity) as GameObject;
	}
...
//	private void CreateGapCube(int z){
//
//		Vector3 position = new Vector3 ( 0, 0.2f, z );
//
//		for (int x = -numCubeWidth; x < numCubeWidth; x++) {
//
//			//Set Y position
//			float difference = Mathf.Abs( x - cliffBridgeNoise[z] );
//
//			if(difference < 1.5f){
//				position.x = cliffBridgeNoise [z];
//				GameObject cube = Instantiate (bridgeCube, position, Quaternion.identity) as GameObject;
//				cube.transform.Rotate(new Vector3(0, -GetBrigdeStepRotation(z, cliffBridgeNoise),0));
//
//				position.x = cliffBridgeNoise2 [z];
//				GameObject cube2 = Instantiate (bridgeCube, position, Quaternion.identity) as GameObject;
//				//Rotate bridge cube
//				cube2.transform.Rotate(new Vector3(0, -GetBrigdeStepRotation(z, cliffBridgeNoise2), 0) );
//			}
//		}
//	}
	...
	/**
	 * Returns the angle between two points in the levelAreaNoise array
	 */
//	private float GetAngleForCube(int sample){
//
//		Vector2 prevPos = new Vector2 ( sample - 1, levelAreaNoise[ sample - 1 ] );
//		Vector2 thisPos = new Vector2 ( sample, levelAreaNoise[ sample ] );
//		Vector2 nextPos = new Vector2 ( sample + 1, levelAreaNoise[ sample + 1 ] );
//
//		float angle1 = Mathf.Atan2 (prevPos.y - thisPos.y, prevPos.x - thisPos.x);
//		float angle2 = Mathf.Atan2 (nextPos.y - thisPos.y, nextPos.x - thisPos.x);
//
